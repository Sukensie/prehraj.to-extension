...
chrome.runtime.sendMessage({ setting: "getAutoPlay" }, function (response) {
    ...
    //play the next video only if autoplay is set to true
    if (response && response.autoPlay) {
        ...
        else //if no video was found - probably last episode of season => increment season number
        {
            ...
            //every qeury the url gets one more '/' so if there are more than 4 (https://prehraj.to/hledej/...) it means there are no more seassons
